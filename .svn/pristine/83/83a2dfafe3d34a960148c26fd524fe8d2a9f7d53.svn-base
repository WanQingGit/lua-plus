...
#define ci_func(ci)		(clLvalue((ci)->func[0]))
...
static const char *findvararg(CallInfo *ci, int n, StkId *pos) {
	int nparams = clLvalue(ci->func[0])->p->numparams;
...
LUA_API const char *lua_getlocal(lua_State *L, const lua_Debug *ar, int n) {
	...
	if (ar == NULL) { /* information about non-active function? */
		if (!isLfunction(fn)) /* not a Lua function? */
			name = NULL;
		else
			/* consider live variables at function start (parameters) */
			name = luaF_getlocalname(clLvalue(fn)->p, n, 0);
...
//    TValue v;
		...
		stack_push(L, t);/* push it on stack */
//    setbvalue(&v, 1);  /* boolean 'true' to be the value of all indices */
		for (i = 0; i < f->l.p->sizelineinfo; i++) /* for all lines with code */
			luaH_setint(L, t, lineinfo[i], boolTrue); /* table[line] = true */
	}
}
...
LUA_API int lua_getinfo(lua_State *L, const char *what, lua_Debug *ar) {
	...
	TValue *fn = *func;
	cl = ttisclosure(fn) ? clvalue(fn) : NULL;
	...
	*name = luaF_getlocalname(p, reg + 1, lastpc);
	...
	if (pc != -1) { /* could find instruction? */
		...
		case OP_GETTABLE: {
			...
			? luaF_getlocalname(p, t + 1, pc) : upvalname(p, t);
			kname(p, pc, k, name);
 ...
 */
static inline int isinstack(CallInfo *ci, StkId o) {
