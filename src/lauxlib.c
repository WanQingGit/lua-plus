...
LUALIB_API int luaL_loadfilex(lua_State *L, const char *filename,
	...
	if (c == LUA_SIGNATURE[0] && filename) { /* binary file? c为读取到的第一个字符*/
	...
	lua_remove(L, fnameindex); //移除@xx/xx.lua
	return status;
}
...
static const char *getS(lua_State *L, void *ud, size_t *size) {
	LoadS *ls = (LoadS *) ud; //ls->s 加了return
	(void) L; /* not used */
	if (ls->size == 0)
		return NULL;
	*size = ls->size;
...
LUALIB_API int luaL_getsubtable(lua_State *L, int idx, const char *fname) {
	Table *t = (Table *) index2addr(L, idx);
	TValue *v;
	if (ttistable(t)) {
		Node res;
		int insert = t->metatable == NULL;
		TString *key = luaS_new(L, fname);
		if (luaH_gset(L, t, (TValue*) key, key->hash, insert, &res)) {
			v = res.map->i_val;
			if (ttistable(v)) {
				stack_push(L, v);
				return 1;
	...
	TString *key = luaS_new(L, fname);
	TValue *slot = ttistable(t) ? luaO_nilobject : NULL;
	luaV_finishget(L, (TValue*) t, (TValue*) key, L->top++, slot);
	v = *(L->top - 1);
	if (ttype(v) == LUA_TTABLE) {
		refInc(v);
		return 1;
	}
	v = (TValue *) luaH_new(L);
	luaV_finishset(L, (TValue*) t, (TValue*) key, v, slot);
	*(L->top - 1) = v;
	refInc(v);
	return 0;
}
...
LUALIB_API void luaL_requiref(lua_State *L, const char *modname,
	...
	if (!lua_toboolean(L, -1)) { /* package not already loaded? */
		...
		lua_pushvalue(L, -1); /* 返回的是table,make copy of module (call result) */
